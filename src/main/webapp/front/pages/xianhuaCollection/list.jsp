                    ...
                    <form class="layui-form" lay-filter="myForm">
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <input type="text" name="xianhuaName" id="xianhuaName" placeholder="鲜花名称" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <select name="xianhuaTypes" id="xianhuaTypes" lay-filter="xianhuaTypes">
                ...
                <div class="ng-inside-box ng-fault ng-container" aos="fade-up">
                    <div class="show-reel">
                        <div v-for="(item,index) in dataList" v-bind:key="index" class="col-md-3 agile-gallery-grid">
                            <i @click="deleteXianhuaCollection(item.id)" class="layui-icon" style="float:right;color:red;margin-top: -10px;font-size:20px">&#xe640;</i>
                            <div class="agile-gallery">
                                <a @click="jumpCheck('../xianhua/detail.jsp?id='+item.xianhuaId , item.xianhuaDelete == null?'':item.xianhuaDelete , item.shangxiaTypes == null?'':item.shangxiaTypes)" class="lsb-preview" data-lsb-group="header">
                                    <img :src="item.xianhuaPhoto?item.xianhuaPhoto.split(',')[0]:''" style="height: 350px;width:100%;" />
                                    <div class="agileits-caption">
                                        <h4>{{item.xianhuaName}}</h4>
                                        <p v-if="item.xianhuaNewMoney != null" >￥{{item.xianhuaNewMoney}}</p>
                                    </div>
                                </a>
                            </div>
                        </div>
...
<script>
    var vue = new Vue({
        ...
        methods: {
            ...
            ,deleteXianhuaCollection(id){
                layui.layer.confirm('确认要删除这个收藏吗？', {
                    btn : [ '确定', '取消' ]//按钮
                }, function(index) {
                    var paramArray = [];
                    paramArray.push(id);
                    layui.http.requestJson('xianhuaCollection/delete', 'post',paramArray, function (res) {
                        if (res.code == 0) {
                            layer.msg('删除成功', {
                                time: 2000,
                                icon: 6
    ...
    // 当前表的字典表
        ...
        function xianhuaCollectionTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=xianhua_collection_types", 'get', {}, function (res) {
                if(res.code == 0){
                    vue.xianhuaCollectionTypesList = res.data.list;
                }
            });
        }
    ...
    // 级联表的字段数据
        ...
        function xianhuaTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=xianhua_types", 'get', {}, function (res) {
                if(res.code == 0){
        ...
        function pageList() {
            ...
            if (jquery('#xianhuaName').val() != null && jquery('#xianhuaName').val() != "" && jquery('#xianhuaName').val() != "null" ) {
                param['xianhuaName'] = jquery('#xianhuaName').val();
            }
                                         
            var xianhuaTypes = $("#xianhuaTypes").find("option:selected").val();
            if (xianhuaTypes != null && xianhuaTypes != "" && xianhuaTypes != "null") {
                            param['xianhuaTypes'] = xianhuaTypes;
                        }
                                                                                                                                                                                    
            // 当前表
                                                
            //收藏只能查类型为1的
            param['xianhuaCollectionTypes'] = 1;

            // 获取列表数据
            http.request('xianhuaCollection/page', 'get', param, function(res) {
