...
<div id="app">
    ...
    <ul class="cart_list_td clearfix" v-for="(item,index) in dataList" v-bind:key="index">
        <li class="col01"></li>
        <li class="col02"><img :src="item.xianhuaPhoto"></li>
        <li class="col03">{{item.xianhuaName}}<br><em>{{item.xianhuaNewMoney}} RMB</em></li>
    ...
    var vue = new Vue({
        ...
        methods: {
			...
			,addCartNum(index) {
				...
				layui.http.request(`xianhua/info/${item.xianhuaId}`, 'get', {}, function(res) {
					// 库存限制
					if (res.data.xianhuaKucunNumber && res.data.xianhuaKucunNumber > 0 && res.data.xianhuaKucunNumber  <= item.buyNumber) {
						layui.layer.msg(`鲜花库存不足`, {
							time: 2000,
							icon: 5
						});
						return
			...
			// 购买
			buyClick() {
				localStorage.setItem('xianhuas', JSON.stringify(this.dataList));
				window.location.href = '../xianhuaOrder/confirm.jsp'
			}
        }
    });
