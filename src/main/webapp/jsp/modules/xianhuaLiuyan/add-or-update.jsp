            ...
            <div class="row breadcrumb-bar">
                ...
                <div class="col-md-6">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active">编辑鲜花留言</li>
                    </ol>
                </div>
            </div>
        </div>
                        ...
                        <form id="addOrUpdateForm">
                            ...
                            <!-- 当前表的字段 -->
                                ...
                                <input id="yonghuId" name="yonghuId" type="hidden">
                                    <div class="form-group col-md-6 xianhuaLiuyanTextDiv">
                                        <label>留言内容</label>
                                        <textarea style="width: 450px"  id="xianhuaLiuyanText" name="xianhuaLiuyanText" class="form-control"placeholder="留言内容"></textarea>
                                    </div>
...
<script>
    ...
    function submit() {
        if (validform() == true && compare() == true) {
            ...
            httpJson("xianhuaLiuyan/" + urlParam, "POST", data, (res) => {
                if(res.code == 0){
    ...
    function validform() {
        return $("#addOrUpdateForm").validate({
            rules: {
                xianhuaId: "required",
                yonghuId: "required",
                xianhuaLiuyanText: "required",
                replyText: "required",
            },
            messages: {
                xianhuaId: "鲜花不能为空",
                yonghuId: "用户不能为空",
                xianhuaLiuyanText: "留言内容不能为空",
                replyText: "回复内容不能为空",
            }
        }).form();
    }
    ...
    // 获取当前详情
    function getDetails() {
        var addxianhuaLiuyan = window.sessionStorage.getItem("addxianhuaLiuyan");
        if (addxianhuaLiuyan != null && addxianhuaLiuyan != "" && addxianhuaLiuyan != "null") {
            //注册表单验证
            $(validform());
            ...
            window.sessionStorage.removeItem('updateId');
            http("xianhuaLiuyan/info/" + updateId, "GET", {}, (res) => {
                if(res.code == 0)
                {
                    ruleForm = res.data
                    // 是/否下拉框回显
    ...
    <!--  当前表的数据回显  -->
        ...
        $("#xianhuaLiuyanText").val(ruleForm.xianhuaLiuyanText);
    ...
    function readonly() {
        if (window.sessionStorage.getItem('role') == '管理员') {
            $('#xianhuaLiuyanText').attr('readonly', 'readonly');
