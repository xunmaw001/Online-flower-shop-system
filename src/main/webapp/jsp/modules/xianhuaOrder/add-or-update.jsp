            ...
            <div class="row breadcrumb-bar">
                ...
                <div class="col-md-6">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active">编辑鲜花订单</li>
                    </ol>
                </div>
            </div>
        </div>
                    ...
                    <div class="widget-area-2 lochana-box-shadow">
                        ...
                        <form id="addOrUpdateForm">
                            ...
                            <!-- 当前表的字段 -->
                                    ...
                                    <div class="form-group col-md-6 xianhuaOrderUuidNumberDiv">
                                        <label>订单号</label>
                                ...
                                <input id="yonghuId" name="yonghuId" type="hidden">
                                    ...
                                    <div class="form-group col-md-6 xianhuaOrderTruePriceDiv">
                                        <label>实付价格</label>
                                        <input style="width: 450px" id="xianhuaOrderTruePrice" name="xianhuaOrderTruePrice" class="form-control"
                                               onchange="xianhuaOrderTruePriceChickValue(this)" placeholder="实付价格">
                                    </div>
                                    <div class="form-group col-md-6 xianhuaOrderTypesDiv">
                                        <label>订单类型</label>
                                        <select style="width: 450px" id="xianhuaOrderTypesSelect" name="xianhuaOrderTypes" class="form-control">
                                        </select>
                                    </div>
                                    <div class="form-group col-md-6 xianhuaOrderPaymentTypesDiv">
                                        <label>支付类型</label>
                                        <select style="width: 450px" id="xianhuaOrderPaymentTypesSelect" name="xianhuaOrderPaymentTypes" class="form-control">
                                        </select>
                                    </div>
                                    <div class="form-group col-md-6 xianhuaOrderKuaididanhaoDiv">
                                        <label>快递单号</label>
...
<script>
    ...
    function submit() {
        if (validform() == true && compare() == true) {
            ...
            httpJson("xianhuaOrder/" + urlParam, "POST", data, (res) => {
                if(res.code == 0){
    ...
    // 查询列表
        ...
        function xianhuaOrderTypesSelect() {
            //填充下拉框选项
            http("dictionary/page?page=1&limit=100&sort=&order=&dicCode=xianhua_order_types", "GET", {}, (res) => {
                if(res.code == 0){
                    xianhuaOrderTypesOptions = res.data.list;
                }
            });
        }
        function xianhuaOrderPaymentTypesSelect() {
            //填充下拉框选项
            http("dictionary/page?page=1&limit=100&sort=&order=&dicCode=xianhua_order_payment_types", "GET", {}, (res) => {
                if(res.code == 0){
                    xianhuaOrderPaymentTypesOptions = res.data.list;
                }
            });
        }
        <!-- 查询级联表的所有列表 -->
    ...
    // 初始化下拉框
    <!-- 初始化当前表的下拉框 -->
        function initializationXianhuaordertypesSelect(){
            var xianhuaOrderTypesSelect = document.getElementById('xianhuaOrderTypesSelect');
            if(xianhuaOrderTypesSelect != null && xianhuaOrderTypesOptions != null  && xianhuaOrderTypesOptions.length > 0 ){
                for (var i = 0; i < xianhuaOrderTypesOptions.length; i++) {
                    xianhuaOrderTypesSelect.add(new Option(xianhuaOrderTypesOptions[i].indexName,xianhuaOrderTypesOptions[i].codeIndex));
                }
            }
        }
        function initializationXianhuaorderpaymenttypesSelect(){
            var xianhuaOrderPaymentTypesSelect = document.getElementById('xianhuaOrderPaymentTypesSelect');
            if(xianhuaOrderPaymentTypesSelect != null && xianhuaOrderPaymentTypesOptions != null  && xianhuaOrderPaymentTypesOptions.length > 0 ){
                for (var i = 0; i < xianhuaOrderPaymentTypesOptions.length; i++) {
                    xianhuaOrderPaymentTypesSelect.add(new Option(xianhuaOrderPaymentTypesOptions[i].indexName,xianhuaOrderPaymentTypesOptions[i].codeIndex));
                }
            }
        }
    ...
    function setSelectOption() {
        ...
        if(xianhuaOrderTypesSelect != null && xianhuaOrderTypesOptions != null  && xianhuaOrderTypesOptions.length > 0 ) {
            for (var i = 0; i < xianhuaOrderTypesOptions.length; i++) {
                if (xianhuaOrderTypesOptions[i].codeIndex == ruleForm.xianhuaOrderTypes) {//下拉框value对比,如果一致就赋值汉字
                        xianhuaOrderTypesSelect.options[i].selected = true;
                }
            }
        }
        ...
        if(xianhuaOrderPaymentTypesSelect != null && xianhuaOrderPaymentTypesOptions != null  && xianhuaOrderPaymentTypesOptions.length > 0 ) {
            for (var i = 0; i < xianhuaOrderPaymentTypesOptions.length; i++) {
                if (xianhuaOrderPaymentTypesOptions[i].codeIndex == ruleForm.xianhuaOrderPaymentTypes) {//下拉框value对比,如果一致就赋值汉字
                        xianhuaOrderPaymentTypesSelect.options[i].selected = true;
                }
            }
        }
        <!--  级联表的下拉框回显  -->
    ...
    //数字检查
        ...
        function xianhuaOrderTruePriceChickValue(e){
            var this_val = e.value || 0;
    ...
    function validform() {
        return $("#addOrUpdateForm").validate({
            rules: {
                xianhuaOrderUuidNumber: "required",
                addressId: "required",
                xianhuaId: "required",
                yonghuId: "required",
                buyNumber: "required",
                xianhuaOrderTruePrice: "required",
                xianhuaOrderTypes: "required",
                xianhuaOrderPaymentTypes: "required",
                xianhuaOrderKuaididanhao: "required",
            },
            messages: {
                xianhuaOrderUuidNumber: "订单号不能为空",
                addressId: "收获地址不能为空",
                xianhuaId: "鲜花不能为空",
                yonghuId: "用户不能为空",
                buyNumber: "购买数量不能为空",
                xianhuaOrderTruePrice: "实付价格不能为空",
                xianhuaOrderTypes: "订单类型不能为空",
                xianhuaOrderPaymentTypes: "支付类型不能为空",
                xianhuaOrderKuaididanhao: "快递单号不能为空",
            }
        }).form();
    }
    ...
    // 获取当前详情
    function getDetails() {
        var addxianhuaOrder = window.sessionStorage.getItem("addxianhuaOrder");
        if (addxianhuaOrder != null && addxianhuaOrder != "" && addxianhuaOrder != "null") {
            //注册表单验证
            $(validform());
            $("#xianhuaOrderUuidNumber").val(new Date().getTime()+Math.ceil(Math.random()*10));//设置唯一号
            ...
            window.sessionStorage.removeItem('updateId');
            http("xianhuaOrder/info/" + updateId, "GET", {}, (res) => {
                if(res.code == 0)
                {
                    ruleForm = res.data
                    // 是/否下拉框回显
    ...
    <!--  当前表的数据回显  -->
        ...
        $("#xianhuaOrderUuidNumber").val(ruleForm.xianhuaOrderUuidNumber);
        $("#addressId").val(ruleForm.addressId);
        $("#xianhuaId").val(ruleForm.xianhuaId);
        $("#yonghuId").val(ruleForm.yonghuId);
        $("#buyNumber").val(ruleForm.buyNumber);
        $("#xianhuaOrderTruePrice").val(ruleForm.xianhuaOrderTruePrice);
        $("#xianhuaOrderKuaididanhao").val(ruleForm.xianhuaOrderKuaididanhao);
    ...
    $(document).ready(function () {
        ...
        // 初始化下拉框
            <!--  初始化当前表的下拉框  -->
            initializationXianhuaordertypesSelect();
            initializationXianhuaorderpaymenttypesSelect();
            <!--  初始化级联表的下拉框  -->
